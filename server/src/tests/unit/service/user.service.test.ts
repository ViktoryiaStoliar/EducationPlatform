...
describe('', () => {
    test('', async () => {
        ...
        mock.mockResolvedValue([{ id: 1, name: 'test1', surname: 'test2', email: 'test@mail.ru', pwd: 'asdfghjkl' }])

        const res = await getAllUser();

        expect(mock).toHaveBeenCalled();
        expect(res.length).toBeGreaterThan(0);
        // expect(res).toEqual([{ id: 1, name: 'test1', surname: 'test2', email: 'test@mail.ru', pwd: 'asdfghjkl'}, { id: 2, name: 'test3', surname: 'test4', email: 'test1@mail.ru', pwd: 'asderthjkl'}])
    })
})
